...
class HomeView extends Backbone.View
  ...
  initialize: ->
    ...
    for model in @collection.models
      ...
      @total_amount += model.get('amount') - paid_amount

      # TODO - validate first
      @weekData[due_date.format('w')][due_date.day()]['bills'].push model
      @weekSummary[due_date.format('w')]['bills'].push model
      @weekSummary[due_date.format('w')]['total_amount'] += model.get('amount') - paid_amount
    ...
    @weekly = new WeekView
      ...
      weekData: @weekData[@today.format('w')]
      weekSummary: @weekSummary[@today.format('w')]
  ...
  render: ->
    ...
    @weekly.render()
    @$weeklySection.append @weekly.$el
